...
#include "Renderer.h"
  ...
  #include "RenderAPI/Vulkan/VulkanShader.h"
#endif
...
#if defined(AGE_INCLUDE_VULKAN)
        return MakeRef<VulkanShader>(name, vertexSrc, fragmentSrc);
#else
        AGE_CORE_ASSERT(false, "Vulkan Render API is not included in the current compilation.");
#endif
      default: {
